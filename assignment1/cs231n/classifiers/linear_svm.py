...
def svm_loss_naive(W, X, y, reg):
    ...
    for i in range(num_train):
        ...
        for j in range(num_classes):
            ...
            if margin > 0:
                loss += margin
                dW[:, y[i]] += -X[i] # dLi / dw_yi = -x_i
                dW[:, j] += X[i] # dLi / dw_j = x_i
...
def svm_loss_vectorized(W, X, y, reg):
    ...
    correct_class_scores = scores[range(X.shape[0]), y] # (500, )
    correct_class_scores = np.reshape(correct_class_scores, (correct_class_scores.shape[0], 1)) # (500, 1)

    delta = np.ones(scores.shape) 
    delta[range(X.shape[0]), y] = 0 # correct class에는 적용하지 않기 위해서.
